...
export class Migration20260630032459 extends Migration {

  override async up(): Promise<void> {
    this.addSql(`alter table if exists "product_ml_link" drop constraint if exists "product_ml_link_ml_item_id_unique";`);
    this.addSql(`alter table if exists "product_ml_link" drop constraint if exists "product_ml_link_product_id_unique";`);
    ...
    this.addSql(`CREATE UNIQUE INDEX IF NOT EXISTS "IDX_product_ml_link_product_id_unique" ON "product_ml_link" ("product_id") WHERE deleted_at IS NULL;`);
    this.addSql(`CREATE UNIQUE INDEX IF NOT EXISTS "IDX_product_ml_link_ml_item_id_unique" ON "product_ml_link" ("ml_item_id") WHERE deleted_at IS NULL;`);
  }
